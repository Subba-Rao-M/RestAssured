...
import api.endpoints.UserEndPointsLinkedwithRoutePropFile;
...
public class UserTestswithPropFile {
	...
	public void test_PostUser(){
		...
		Response response = UserEndPointsLinkedwithRoutePropFile.createUser(userPayload);
	...
	public void test_getUserByName() {
		...
		Response response = UserEndPointsLinkedwithRoutePropFile.getUser(this.userPayload.getUsername());
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(), 200, "Status code is not matching");
	...
	public void test_updateUserByUserName() {
		...
		userPayload.setLastName(faker.name().lastName());
		userPayload.setPhone(faker.phoneNumber().cellPhone());
		Response response = UserEndPointsLinkedwithRoutePropFile.updateUser(this.userPayload.getUsername(),userPayload );
		response.then().log().all();
		//response.then().log().body().statusCode(200); //One more way of assertion
		Assert.assertEquals(response.getStatusCode(), 200, "Status code is not matching");
		logger.info("Updated user info");
		//checking data after update
		Response updatedResponse = UserEndPointsLinkedwithRoutePropFile.getUser(this.userPayload.getUsername());
	...
	public void test_deleteUserByName() {
		...
		Response response = UserEndPointsLinkedwithRoutePropFile.deleteUser(this.userPayload.getUsername());
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(), 200, "Status code is not matching");
